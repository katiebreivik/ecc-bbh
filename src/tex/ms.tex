...
\subsection{The effect of eccentricity on orbital evolution}
\label{sec:ecc}
 Averaging the energy from GW emission over the binary orbit, we can write the orbital separation and eccentricity evolution as a function of time as
...
\noindent Using Kepler's third law, the orbital separation time evolution can be converted to an orbital frequency evolution 
...
The equations for $de/dt$ and $da/dt$ can be solved for $a(e)$ as 

\begin{equation}
    a(e) = \frac{c_0e^{12/19}}{(1-e^2)}\Big[1 + \frac{121/304}e^2\Big]^{870/2299},
    \label{eq:a_of_e}
\end{equation}
\noindent where $c_0$ is an integration constant found by evaluating equation~\ref{eq:a_of_e} at an initial semimajor axis and eccentricity, $a_0$ and $e_0$ respectively. 

\noindent For binary orbits with appreciable eccentricity ($e\gtrsim0.1$), the orbital contraction due to GW emission is significantly expedited. The merger time for a binary with specified frequency, masses, and eccentricity can be obtained by numerically integrating equation~\ref{eq:dedt} as 

\begin{equation}
    T_{\rm{merge}}(a_0, e_0) = \frac{12}{19}\frac{c_0^4}{\beta} \int \frac{de e^{29/19} [1 + (121/304)e^2]^{1181/2299}}{(1-e^2)^{3/2}},
    \label{eq:T_merge}
\end{equation}

\noindent where $c_0$ is the integration constant in equation~\ref{eq:a_of_e} and $\beta$ is
...
Alternatively, for the non-integration-interested reader, a convenient fit obtained by \cite{Mandel2021} is available over eccentricity ranges between $0$ and $0.99999$ as a modification to the circular merger time, $T_c$,  
\begin{equation}
    T_{\rm{merge}} \simeq T_c (1 + 0.27e_0^10 + 0.33e_0^20 + 0.2e_0^1000)(1-e_0)^{7/2}.
    \label{eq:beta}
\end{equation}

\noindent For the purposes of this study, this fit is perfectly adequate and is thus applied in place of equation~\ref{eq:T_merge} for computational efficiency.

Eccentricity also affects the horizon at which GW signals become resolvable by LISA. On one hand, eccentricity leads to GW emission at potentially several integer harmonics of the orbital frequency as described in \cite{Peters1963}. For binaries with low orbital frequencies ($f_{\rm{orb}}\lesssim 1\,\rm{mHz}$), this `Fourier comb' of emission can significantly increase the detectability of individual binaries. However, for higher frequencies the GW emission can be predominantly produced at frequencies \emph{above} LISA's frequency cutoff near $f_{\rm{GW}}\sim0.1\,\rm{Hz}$. On top of this, the orbital evolution acceleration effect from large eccentricities can cause a given binary to sweep through the LISA band rapidly such that the detector is unable to build up an appreciable signal at a given frequency. 
...
and orbital frequency $f$ merges.  Then the compact binary rate per unit frequency at any given time is
