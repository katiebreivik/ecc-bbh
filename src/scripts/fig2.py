...
ax1.minorticks_on()
ax1.set_ylabel(r'$\mathrm{d}N_{\rm{intrinsic}} / \mathrm{d}M_{\mathrm{BH,1}}$ [$\rm{Gpc}^{-3}\,M_{\odot}^{-1}$]')
ax1.set_xlabel(r'$M_{\mathrm{BH,1}}$ [$M_{\odot}$]')

ax2 = ax1.twinx()

# Plot data on the second y-axis
color = 'navy'
ax2.set_ylabel(r'$\mathrm{d}N_{\rm{detected}} / \mathrm{d}M_{\mathrm{BH,1}}$ $[M_{\odot}^{-1}$]')
for ii, dNdm1 in enumerate(dN_dm1_list):
    ax2.plot(mass_1, dNdm1, color=cs2[ii], label=labels_snr[ii])
ax2.set_ylim(1e-4, 5e2)
ax2.set_xlim(0, 95)
ax2.set_yscale('log')
...
plt.savefig(paths.figures / 'fig1.pdf', dpi=150, facecolor='white')